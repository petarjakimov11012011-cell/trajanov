...
- `src/app/contact/page.tsx` — **contact page (1.06)**, static server component: Bebas `CONTACT` header, one plain intro line, then the channels — verified Instagram link (`@trajanovv2026`) plus email and phone as visible placeholder tokens (D-1.06-1). No street address, no owner name, no form; `metadata` title/description
