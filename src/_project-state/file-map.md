...
- `src/app/layout.tsx` — root layout: loads **Syne** (display) + Hanken Grotesk (body) via `next/font` (Syne replaced Bebas Neue at 1.02c — D-1.02c-2), sets brand `<title>`/description, imports `globals.css`, wraps every route with `<CartProvider>` (1.04a) then `<Header>` + `<Footer>` (min-h-dvh flex column). 1.08: Hanken subset stays `latin` — it has no `cyrillic` Google Fonts subset (D-1.08-6); a comment documents why so it isn't "fixed" into a broken build
...
- `src/app/about/page.tsx` — **about page (1.08, NEW)**, static server component: Syne `ABOUT` title, the founder's story (Vladimir Trajanov · "from Macedonia" · First place, Creative Day × EAM, 2026 — every claim VERIFIED in `facts.md` → Founder / About; nickname/birth-year/school/home-town omitted by child-safety policy), a quiet slogan pull-quote holding the Macedonian-slogan `<PlaceholderToken>` (UNVERIFIED, D-1.08-6) set in the body font, and a `VIEW CATALOG` link → `/products` (`buttonVariants`, D-1.03-1); `metadata` title/description + Open Graph. No photograph (minor — D-1.08-3)
...
- `src/components/header.tsx` — shared site header (1.03; badge added 1.04a; **ABOUT nav added 1.08** — D-1.08-5), client component: sticky Ink bar with hairline, `TRAJANOV` wordmark → `/`, `CATALOG`/`ABOUT`/`CONTACT` nav, cart icon → `/cart` now with a live monochrome item-count badge (reads `useCart()`), and a focus-trapped mobile menu panel (Lucide `Menu`/`X`/`ShoppingBag`)
- `src/components/footer.tsx` — shared site footer (1.03; **renewed 1.02c** from the 1.02b-3 handover; **ABOUT added to the bottom bar 1.08** — D-1.08-5): Surface block, monochrome top hairline (no red — D-1.02c-7), restrained Syne `TRAJANOV` wordmark, two columns — CONTACT (`Mail` icon + email `<PlaceholderToken>`) and SOCIAL (purpose-built Instagram SVG + `@trajanovv2026`; D-1.02c-8) — and a bottom secondary-nav bar (`© 2026 Trajanov` · `ABOUT`/`PRIVACY`/`TERMS`) above a 1px `--border` rule
- `src/components/placeholder-token.tsx` — **shared placeholder token (1.06)**: renders a literal UNVERIFIED-fact token (e.g. `[PLACEHOLDER: public email — from Vladimir]`) in the brand's error colours (`--error` on `--error-surface`, the AA-checked 5.5:1 pair) so it reads as loud and clearly unfinished; used by the about/contact/privacy/terms pages; `box-decoration-clone` keeps the background intact when a long token wraps (no 375px overflow)
